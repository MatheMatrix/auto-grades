...
def WelcomeText():
	print('欢迎使用长大教务管理系统助手v0.2.0版'.decode('utf-8').encode('gbk'))
	print('*'*40)
...
data = GetUsers()
if len(data) == 0:
	print('本地暂无用户数据保存，请输入新数据: '.decode('utf-8').encode('gbk'))
	...
	print('已有 {0} 个用户的数据，选择其中一个来查询（输入后按下回车键）: '.format(int(data['users'])).decode('utf-8').encode('gbk'))
	for i in range(1, int(data['users']) + 1):
		print('{0}. {1}'.format(i, data['user' + str(i)]))
	print('{0}. 输入一份新的数据'.format(i + 1, ).decode('utf-8').encode('gbk'))
	choice = raw_input('输入 \'1\' 或者别的数字（输入后按下回车键）: '.decode('utf-8').encode('gbk'))
	if choice == str(i + 1):
		username = raw_input('输入你的学号然后按下回车键: \n'.decode('utf-8').encode('gbk'))
		password = raw_input('输入你的教务处密码然后按下回车键:  \n'.decode('utf-8').encode('gbk'))
		data['users'] = i + 1
...
print('正在从教务处获得数据...'.decode('utf-8').encode('gbk'))
with open('out.htm', 'w') as f:
	f.write(c.read())
if platform.system() == 'Windows':
	print('正在打开成绩查询页面...'.decode('utf-8').encode('gbk'))
	os.system('out.htm')
